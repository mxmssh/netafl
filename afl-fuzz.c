...
typedef int (APIENTRY* cnc_run)(char*);
typedef int (APIENTRY* cnc_init)(char*);
...
// custom CnC function
cnc_run cnc_run_ptr = NULL;
cnc_init cnc_init_ptr = NULL;
...
DWORD WINAPI process_test_case(LPVOID lpParam)
{
	int result = 0;
	/* open generated file */
	s32 fd = out_fd;
	...
	result = cnc_run_ptr(buf);

	free(buf);

	if (result == 0)
		FATAL("Unable to process test case, the user-defined CnC function return 0");
	return 1;
}
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (enable_server_mode && cnc_run_ptr == NULL) {
    if (!initiate_server())
      return -1;
  ...
  if (enable_socket_fuzzing && !enable_server_mode && cnc_run_ptr == NULL) /* in case of server mode we have special thread created below */
    open_file_and_send_data();

  if (cnc_run_ptr != NULL)
	  CreateThread(NULL, 0, process_test_case, NULL, 0, &dwThreadId);
...
void load_custom_cnc_library(const char *libname) {
	int result = 0;
	SAYF("Loading user-defined CnC library\n");
	HMODULE hLib = LoadLibraryA(libname);
	if (hLib == NULL)
		FATAL("Unable to load custom CnC library, GetLastError = 0x%x", GetLastError());

	/* init the custom CnC server */
	//Get pointer to user-defined CnC function using GetProcAddress:
	cnc_init_ptr = (cnc_init)GetProcAddress(hLib, "_cnc_init@4");
	if (cnc_init_ptr == NULL)
		FATAL("Unable to load init_cnc from the DLL provided by user");
	result = cnc_init_ptr(server_bind_port);

	if (result == 0)
		FATAL("user-defined init_cnc returns 0 which is considered as an error");

	//Get pointer to user-defined CnC function using GetProcAddress:
	cnc_run_ptr = (cnc_run)GetProcAddress(hLib, "_cnc_run@4");
	if (cnc_run_ptr == NULL)
		FATAL("Unable to load run_cnc from the DLL provided by user");
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dYnCB:S:M:x:QD:b:Ua:p:w:s:L:l:")) > 0)
    ...
    switch (opt) {
	  ...
	  case 'L':
        ...
        break;
	  case 'l':
        load_custom_cnc_library(optarg);
